"""Plot potential energy surface from Clifford shadow tomography data.

Loads results from a directory containing data.npz and metadata.json
(as produced by clifford.py) and plots the estimated energy with
variance-based error bars against interatomic distance, compared to
exact FCI and Hartree-Fock reference curves.

Usage:
    python scripts/tomography/clifford/plot_pes.py <results_directory>

Example:
    python scripts/tomography/clifford/plot_pes.py results/tomography/clifford/pes/2026-02-23_12-00-00/
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot PES from Clifford shadow tomography results directory."
    )
